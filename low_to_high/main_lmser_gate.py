...
import data_set
import Generator_lmser_gate as Generator
...
def train(cuda, resume=False, paths=None):
    ...
    label = None
    sample = None
    for epo in range(start_epoch, hyper['epoch']-start_epoch):
        ...
        for npy_index in range(10):
            ...
            save_img(fake_sample.data.cpu().numpy(), str(epo + 1) + '_res_' + str(npy_index + 1))
            save_img(sample.data.cpu().numpy(), str(epo + 1) + '_sample_' + str(npy_index + 1))
            save_img(label.data.cpu().numpy(), str(epo + 1) + '_label_' + str(npy_index + 1))
...
if __name__ == '__main__':
    # model_paths = ['/home/lipeiying/program/_SR_/Lmser_GAN/low_to_high/checkpoint_low_high/model/generator_72.pth',
    #                '/home/lipeiying/program/_SR_/Lmser_GAN/low_to_high/checkpoint_low_high/model/discriminator_72.pth']
    # train(resume=True, paths=model_paths)

    hyper['ckpt_dir'] = 'checkpoint_lmser_gate'
