...
class Lmser_up(nn.Module):
    def __init__(self):
        super(Lmser_up, self).__init__()
        self.up_process = nn.Conv2d(in_channels=3, out_channels=64, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, HR_sample):
        out = self.up_process(HR_sample)
        # print(out.shape)
        ...
        up_list = [up0, up1, up2, up3, up6]
        return LR_out, up_list


class Lmser_down(nn.Module):
    def __init__(self):
        super(Lmser_down, self).__init__()
    ...
    def forward(self, LR_out, up_list):
        out = self.down_process(LR_out)

        out = self.down_layer_0([out, up_list[-1]])
        out = self.down_rescale_0(out)

        # out = self.down_layer_1([out, up_list[-2]])
        # out = self.down_rescale_1(out)
        #
        # out = self.down_layer_2([out, up_list[-3]])
        # out = self.down_rescale_2(out)

        out = self.down_layer_3([out, up_list[-2]])
        out = self.down_rescale_3(out)

        out = self.down_layer_4([out, up_list[-3]])
        out = self.down_rescale_4(out)

        out = self.down_layer_5([out, up_list[-4]])
        out = self.down_rescale_5(out)

        out = self.down_layer_6([out, up_list[-5]])
